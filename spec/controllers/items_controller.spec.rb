...
describe 'ItemsController', type: :request do
  ...
  describe 'update' do
    ...
    it 'updated successfully' do
      ...
      item = JSON.parse(response.body)['item']
      expect(response.status).to eq 200
      expect(item["price"].to_i).to eq 9
    end
  end
  ...
  #   let(:new_item1) { Item.create!({ code: 'TSHIRT', name: 'Roadster', price: 15 }) }
  #   let(:new_item2) { Item.create!({ code: 'TSHIRT', name: 'Roadster', price: 15 }) }
  #   let(:new_item3) { Item.create!({ code: 'TSHIRT', name: 'Roadster', price: 15 }) }
  #   let(:new_item4) { Item.create!({ code: 'TSHIRT', name: 'Roadster', price: 15 }) }
  #   let(:new_item5) { Item.create!({ code: 'MUG', name: 'Mast Harb', price: 6 }) }
  #   let(:new_item6) { Item.create!({ code: 'MUG', name: 'Mast Harb', price: 6 }) }
  #   let(:new_item7) { Item.create!({ code: 'HOODIE', name: 'Super Dry', price: 20 }) }
  #   let(:params) { { items: [new_item1, new_item2, new_item3, new_item4, new_item5,new_item6, new_item7 ]  } }
  #   it 'successfully place the order' do
  #     get 'items/place_order', params: params
